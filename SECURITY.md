...
sequenceDiagram
    ...
    Note over S: session in memory, expires in 12 h: name, email, token source
    S-->>B: 302 /, Set-Cookie sid (HttpOnly, Secure, SameSite=Lax)
```

Tokens stop at the server. The browser only ever receives a random session
...
sequenceDiagram
    ...
    Note over S: sid resolves to the session; older than 12 h: 401
...
|---|---|---|---|
| `sid` cookie: random 256-bit id, HttpOnly, Secure, SameSite=Lax | sessions in process memory, one per login, dropped after 12 h, on logout, or at restart | issues `id_token`, `access_token` for `database.windows.net`, `refresh_token` | validates the token, maps it to the user's Entra identity |
...
| session older than 12 h | dropped on the next request, pools closed, 401 |
